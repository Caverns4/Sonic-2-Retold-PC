...
func getCurrentCamera2D() -> Camera2D:
	...
	if not viewport:
		return null
	var camerasGroupName: String = "__cameras_%d" % viewport.get_viewport_rid().get_id()
...
func get_chaos_emerald_count(value: int) -> int:
	var index: int = 1
...
func convert_player_mode_to_players(mode: int) -> void:
	if mode == 0:
...
func div_by_delta(delta: float) -> float:
...
func get_nearest_player_x(x_pos: float) -> Node2D:
	var closest: float = 64000 #closest x distance
	var finalObj: Node2D = null #Output object
	for player in Global.players: #number of applicable players
		var result: float = absf(x_pos - (player.global_position.x))
...
func get_nearest_player(obj_pos: Vector2) -> Node2D:
	var closest: float = 64000 #closest distance
	var finalObj: Node2D = null #Output object
	for player in Global.players: #number of applicable players
		var result: float = obj_pos.distance_to(player.global_position)
		if !closest or closest > result:
			closest = result
			finalObj = player
	return finalObj
...
func get_orientation_to_player(obj_pos: Vector2)->Vector2:
	if Global.players:
		var player: Player2D = get_nearest_player(obj_pos)
