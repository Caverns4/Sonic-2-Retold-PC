...
func _on_sfx_volume_button_pressed() -> void:
	...
	var volume_to_int: int = int(((AudioServer.get_bus_volume_db(AudioServer.get_bus_index("SFX"))-clampSounds[0])/(abs(clampSounds[0])+abs(clampSounds[1])))*100)
	...
	add_child(sfx_menu)
	var new_volume: int = await sfx_menu.menu_exit
...
func _on_scale_button_pressed() -> void:
	...
	add_child(sfx_menu)
	var new_scale: int = await sfx_menu.menu_exit
